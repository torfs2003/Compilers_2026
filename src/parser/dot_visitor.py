...
class DOTVisitor(BaseVisitor):
    ...
    def _add_node(self, node, label, shape="box", fillcolor="white", style="filled", color="black", fontcolor="black"):
        ...
        if my_id:
            self.dot_content.append(
                f'  {my_id} [label="{label}", shape={shape}, style="{style}", fillcolor="{fillcolor}", color="{color}", fontcolor="{fontcolor}"];')
    ...
    def pre_visit_IncludeNode(self, node):
        self.node_ids[id(node)] = self.generate_id()

    def pre_visit_CommentNode(self, node):
        self.node_ids[id(node)] = self.generate_id()
    ...
    def pre_visit_AssignNode(self, node):
        self.node_ids[id(node)] = self.generate_id()

    def pre_visit_BinOpNode(self, node):
        self.node_ids[id(node)] = self.generate_id()

    def pre_visit_UnaryOpNode(self, node):
        self.node_ids[id(node)] = self.generate_id()

    def pre_visit_CastNode(self, node):
        self.node_ids[id(node)] = self.generate_id()

    def pre_visit_FuncCallNode(self, node):
        self.node_ids[id(node)] = self.generate_id()

    def pre_visit_ReturnNode(self, node):
        self.node_ids[id(node)] = self.generate_id()

    def pre_visit_FunctionDeclNode(self, node):
        self.node_ids[id(node)] = self.generate_id()

    def pre_visit_ArrayInitNode(self, node):
        self.node_ids[id(node)] = self.generate_id()

    def pre_visit_IdentifierNode(self, node):
        self.node_ids[id(node)] = self.generate_id()

    def pre_visit_IntNode(self, node):
        self.node_ids[id(node)] = self.generate_id()

    def pre_visit_FloatNode(self, node):
        self.node_ids[id(node)] = self.generate_id()

    def pre_visit_CharNode(self, node):
        self.node_ids[id(node)] = self.generate_id()

    def pre_visit_StringNode(self, node):
        self.node_ids[id(node)] = self.generate_id()

    def pre_visit_IfNode(self, node):
        self.node_ids[id(node)] = self.generate_id()

    def pre_visit_WhileNode(self, node):
        self.node_ids[id(node)] = self.generate_id()

    def pre_visit_BreakNode(self, node):
        self.node_ids[id(node)] = self.generate_id()

    def pre_visit_ContinueNode(self, node):
        self.node_ids[id(node)] = self.generate_id()

    def pre_visit_SwitchNode(self, node):
        self.node_ids[id(node)] = self.generate_id()

    def pre_visit_EnumNode(self, node):
        self.node_ids[id(node)] = self.generate_id()

    def pre_visit_TypedefNode(self, node):
        self.node_ids[id(node)] = self.generate_id()

    def pre_visit_StructDeclNode(self, node):
        self.node_ids[id(node)] = self.generate_id()

    def pre_visit_MemberAccessNode(self, node):
        self.node_ids[id(node)] = self.generate_id()
