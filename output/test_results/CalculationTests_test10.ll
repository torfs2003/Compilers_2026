...
entry:
  ...
  %".14" = ptrtoint i32* %"x_ptr_load" to i32
  ...
  %"deref_load.1" = load i32, i32* %"z_load"
  ;  Source: x
  %"x_load.1" = load i32, i32* %"x"
  ;  Source: *z==x
  %".24" = icmp eq i32 %"deref_load.1", %"x_load.1"
  ...
  %"deref_load.2" = load float, float* %"a_ptr_load"
  ;  Source: a
  %"a_load" = load float, float* %"a"
  ;  Source: (*a_ptr==a)
  %".34" = fcmp oeq float %"deref_load.2", %"a_load"
